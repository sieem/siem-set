...
export const countScore = ((activatedCardIds: string[]) => {
    ...
    score$.update((currentScore) => {
        ...
        let hintsGiven: number;
        hintsGiven$.subscribe((value) => hintsGiven = value)();

        const newScore = (1 / (currentTime - getLastTimePairFound() + 1)) * pairScore * 10;
        const hintPenalty = [1, 2/3, 1/3, 0];

        return currentScore + Math.round(newScore * hintPenalty[hintsGiven]);
    });
...
export const insertScore = async ({ score, time, unusedCards }: Pick<ScoresTables, 'score' | 'time' | 'unusedCards'>) => {
    const lastRecordDateTime = Date.now();
    lastRecordDateTime$.set(lastRecordDateTime);
    await database.table('scores').put({ score, time, unusedCards, date: lastRecordDateTime});
