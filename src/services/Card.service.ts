...
import { activatedCards, amountOfCards, cards, cardsOnTheTable } from './Card.store';
...
const getCardId = ({ amount, color, filling, shape }: ICard): string =>
...
const checkSet = (cards: ICard[]): boolean => {
  ...
  for (let i = 0; i < 4; i++) {
    const numberToCheck = cardsSplit
      .map((el) => el[i])
      .sort()
      .join('');
...
activatedCards.subscribe(async (_activatedCards: ICard[]) => {
  ...
  cardsOnTheTable.update((cardsOnTheTable) =>
    cardsOnTheTable.map((cardOnTheTable) => ({ ...cardOnTheTable, active: false })),
  );
  ...
  if (checkSet(_activatedCards)) {
    ...
    updatedCardsOnTheTable = oldCardsOnTheTable.filter(
      (cardOnTheTable) => !getCardIds(_activatedCards).includes(getCardId(cardOnTheTable)),
    );
    ...
    cardsOnTheTable.update((cardsOnTheTable) =>
      cardsOnTheTable.map((cardOnTheTable) =>
        activatedCardsIds.includes(getCardId(cardOnTheTable))
    ...
    cardsOnTheTable.update((cardsOnTheTable) =>
      cardsOnTheTable.map((cardOnTheTable) => ({ ...cardOnTheTable, wrong: false })),
    );
  }
});
