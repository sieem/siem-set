...
import { getLastTimePairFound, setLastTimePairFound, time } from './Timer.service';
export const score = writeableWithValue(0);
export const scoreBoard = writable<{ score: ScoresTables[]; time: ScoresTables[] }>({
  score: [],
  time: [],
});
...
export const insertScore = async ({
  ...
  time,
  unusedCards,
}: Pick<ScoresTables, 'score' | 'time' | 'unusedCards'>) => {
...
export const setLatestHighScore = async () =>
